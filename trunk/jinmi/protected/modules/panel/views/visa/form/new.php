            ...
            <td>
                ...
                if(PanelUser::checkAttributesAccess('country', $model)){
                    ...
                    echo $form->dropDownList($model, 'country',
                        CHtml::listData(Address::model()->findAll('parent_id > 0'), 'id', 'name', 'parent.name'), array('readonly'=> PanelUser::checkAttributesAccess('country', $model))
                    );
                }
                ?>
                    ...
                    foreach($model->customer as $key=>$v){
                        ...
                        if(!PanelUser::checkAttributesAccess('customer', $model)){
                            ...
                            echo empty($v->agencyType) ? ''.(CHtml::dropDownList('VisaOrderCustomer['.$v->id.'][agency_id]', 0,
                                    CHtml::listData(VisaTypeAgency::model()->findAllByAttributes(array('type_id'=>$model->type)), 'id', 'agency.name'))) : $v->agencyType->agency->name;
                            echo '</td>';
                            if($v->status == VisaOrderCustomer::STATUS_DEFAULT){
                                if(empty($v->is_pay_out) && empty($v->is_pay)){
                                    echo '<td><a href="#" class="btn btn-default btn-xs btn-danger deleteThis">删除</a>&nbsp;<a href="#" data-customer-id="'.$v->id.'" class="btn btn-default btn-xs btn-info sentVisa">送签</a></td>';
                                }else{
                                    echo '<a href="#" data-customer-id="'.$v->id.'" class="btn btn-default btn-xs btn-info sentVisa">送签</a></td>';
                                }

                            }elseif($v->status == VisaOrderCustomer::STATUS_SENTOUT){
                                if(empty($v->is_pay_out) && empty($v->is_pay)){
                                    echo '<td><a href="#" class="btn btn-default btn-xs btn-danger deleteThis">删除</a>&nbsp;<a href="#" data-customer-id="'.$v->id.'" class="btn btn-default btn-xs btn-info confirmIssued">出签</a>&nbsp;<a href="#" data-customer-id="'.$v->id.'" class="btn btn-default btn-xs btn-danger rejectVisa">拒签</a></td>';
                                }else{
                                    echo '<td><a href="#" data-customer-id="'.$v->id.'" class="btn btn-default btn-xs btn-info confirmIssued">出签</a>&nbsp;<a href="#" data-customer-id="'.$v->id.'" class="btn btn-default btn-xs btn-danger rejectVisa">拒签</a></td>';
                                }
