...
class PHPWord_Writer_Word2007_Base extends PHPWord_Writer_Word2007_WriterPart {
	...
	protected function _writeParagraphStyle(PHPWord_Shared_XMLWriter $objWriter = null, PHPWord_Style_Paragraph $style, $withoutPPR = false) {
		...
		//添加文本缩进2012-07-17 WL 参照 http://blog.csdn.net/yukirin_fans/article/details/7358382
		$indentLeft            = $style->getIndentLeft(); //左缩进值 单位(twips)
		$indentRight           = $style->getIndentRight(); //右缩进值 单位(twips)
 		$indentFirstLine       = $style->getIndentFirstLine(); //首行缩进值 单位(twips)
		$indentFirstLineChars  = $style->getIndentFirstLineChars(); //首行字符缩进值 单位(字符)
		//添加文本缩进结束
...
//				$objWriter->endElement();
//			}
//			//add bgColor 参照https://github.com/Voziv/PHPWord/blob/44d74ef304d9212f129fdff8709779cfcf778660/library/PHPWord/Writer/Word2007/Base.php
//			if(!is_null($bgColor)) {
//			    $objWriter->startElement('w:shd');
//				    $objWriter->writeAttribute('w:val','clear');
//					$objWriter->writeAttribute('w:color','auto');
...
//					    $objWriter->writeAttribute('w:val','single');
//						$objWriter->writeAttribute('w:sz',$brdSz[0]);
//						$objWriter->writeArrtibute('w:color',$brdCol[0]);
//					$objWriter->endElement();
//				}
//				
//				if($bLeft){
...
//	   */
	   ...
	   //段落缩进 2012-07-17 WL 参照 http://blog.csdn.net/yukirin_fans/article/details/7358382
	...
	protected function _writePreserveText(PHPWord_Shared_XMLWriter $objWriter = null, PHPWord_Section_Footer_PreserveText $textrun) {
		...
		$objWriter->startElement('w:p');
			...
			foreach($arrText as $text) {
				...
				if(substr($text, 0, 1) == '{') {//给word加页码时，Page {PAGE} of {NUMPAGES}
	...
	protected function _writeCellStyle(PHPWord_Shared_XMLWriter $objWriter = null, PHPWord_Style_Cell $style = null) {
		...
		//添加合并单元格 WL2012-07-30 参照http://phpword.codeplex.com/discussions/236364 不理想
		$gridSpanCol = $style->getgridSpanCol();
		$vMergeRow   = $style->getVMergeRow();
		//添加结束
		...
		$styles = ( !is_null($bgColor) || !is_null($valign) || !is_null($textDir) || $borders || !is_null($gridSpanCol) || !is_null($vMergeRow) ) ? true : false;
		
		if($styles) {
		
		   //Add colspan And rowspan
		   if(!is_null($gridSpanCol)) {
		        $objWriter->startElement('w:gridSpanRCol');
				    $objWriter->writerAttribute('w:val',$gridSpanCol);
				$objWriter->endElement();
		   }
		   
		   if(!is_null($vMergeRow)) {
		        $objWriter->startElement('w:vMergeRow');
				    $objWriter->writerAttribute('w:val',$vMergeRow);
			    $objWriter->endElement(); 
