(function($){
    $(document).ready(function(){
        ...
        var ChatLoop = function(){
            this.bodyholder = '#chat-body';
            this.bodyholder$ = $(this.bodyholder);
            this.active = '';
            this.init();
        };
        ChatLoop.prototype.init = function(){
            this.startInterval();
        };
        ChatLoop.prototype.startInterval = function(){

        };
        ChatLoop.prototype.loadMessage = function(){
            $.ajax({
                url: 'index.php?r=site/chatdata',
                data: {action: 'get'},
                type: 'POST',
                dataType: 'json',
                cache: false,
        ...
        ChatLoop.prototype.loadSuccess = function(data){
            this.loopMessages(data);
        };
        ...
        ChatWidget.prototype.loopMessages = function(data){
            $(data).each(function(){

                tempNew = '#'+this.username;
                if(this.bodyholder$.has(tempNew)){
                    tempNew$ = $(tempNew);
                    tempNew$.find('.js-chat-messages').html('');
                }else{
        ...
        ChatLoop.prototype.checkNewMsg = function(){

        };
        ChatLoop.prototype.createChatWindow = function(){

        };
        ChatLoop.prototype.deleteChatWindow = function(){

        }
        var chatWidget, ChatWidget = function(userId){
            this.el = '#'+userId;
            this.el$ = $(this.el);
            this.delay = $(this.el).data('delay') || 100;
