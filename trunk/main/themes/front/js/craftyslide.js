...
(function ($) {
    $.fn.craftyslide = function (options) {
        ...
        return this.each(function () {
            ...
            function autoDo() {
                $("#pagination li a").removeClass("active").filter('[href="#'+$slides.filter(':visible').attr('id')+'"]').addClass('active');
                timeout= setTimeout(function () {
                    if (($slides.filter(":visible").index()+1) < ($slides.size())) {

                        $slides.filter(":visible").hide().next("li").fadeIn(options.fadetime);
