$(document).ready(function() {
	...
	$('#addPolicy').click(function(event) {
		...
		var cloneLeftPolicy= $('div#policy:first div#leftPolicy').clone();
		var clonePolicy= $('div#policy:first').clone(true);
		...
		clonePolicy.find('textarea:first').attr('id', 'includeEditor'+(policys+1));
		clonePolicy.find('textarea:last').attr('id', 'notIncludeEditor'+(policys+1));
		clonePolicy.find('input[type !="button"],textarea').each(function() {
			var oldName= $(this).attr('name');
		...
		clonePolicy.find('input[name*="[ItemAttribute][3][value]"]').remove();
		clonePolicy.find('input[name*="[ItemAttribute][4][value]"]').remove();
		...
		var id= $('input[name="'+timeName+'"]').attr('id');;
		$("input#"+id).datepicker();
		timeName= 'ItemForm[Policy]['+policys+'][ItemAttribute][4][value]';
		insert= "<input type='text' id='policy"+(policys+1)+"endtime' name='"+timeName+"'/>";
		$(insert).insertAfter('label[for="'+timeName+'"]');	
		id= $('input[name="'+timeName+'"]').attr('id');;
		$("input#"+id).datepicker();
	...
	$('input.addAdult').click(function(event) {
		...
		var adult= $('div.'+parentID).find('tr.priceItemAdult').size();
		...
		clone.find('input[type!="button"]').each(function() {
			var oldName= $(this).attr('name');
			var newName= oldName.replace(regReplace,'[ItemPrice]['+(adult)+']');//??
			$(this).attr('name', newName);
		});
	...
	$('input.removeChild').click(function(event) {
		var regReplace= /\[ItemPrice\]\[\d+\]/g;
		var parentID= $(this).parents('div#policy').attr('class');
		var curNum= $(this).parents('tr.priceItemChild').find('input[type="hidden"]').attr('name');
		curNum= parseInt(curNum.slice(31, 32));//提取数字
		if (curNum != 1) 
			{
				$('div.'+parentID+' tr.priceItemChild,div.'+parentID+' tr.priceItemAdult').each(function() {
					var num= $(this).find('input[type="hidden"]').attr('name');
					num= parseInt(num.slice(31, 32));
					if (num > curNum) {
						var oldName= $(this).find('input[type != "button"]').attr('name');
	...
	$('input#addContent').click(function(event) {
		...
		var inputName= clone.find('input:text').attr('name').replace(regReplace,'[ItemSection]['+(contents)+']');
		clone.find('label,input:text').remove();
